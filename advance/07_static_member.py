class Robot:
    ...
    # 2. self는 내가 소속된 객체를 의미하는데 원본에서 왔으므로 객체가 없다.
    # 3. @classmethod 어노테이션을 이용해서 원본에서 직접 왔으므로 self 는 객체가 아닌 클래스라고 알려줌
    # 4. 그러니 self 라는 이름은 객체를 받는 인자값으로 오해할수 있으니 cls로 바꾸라고 권고
...
r1 = Robot()
r2 = Robot()
# r1 과 r2는 서로 다른 객체이므로 count를 건드렸을때 서로 영향받지 않는다.
r1.count += 1
